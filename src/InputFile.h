...
namespace deusbmux {
    ...
    class DeviceHandle {
    public:
        DeviceHandle(uint64_t id, uint16_t vendorId, uint16_t productId) {
            this->m_device = std::make_unique<Device>(id, vendorId, productId);
        }

        void beginControl(uint8_t endpoint, uint64_t id, usb_setup_t* setup) {
            if (this->m_inflight[endpoint].contains(id)) {
                std::cerr << "Already had inflight operation on " << std::dec << (uint32_t)endpoint << " with id " << std::hex << id << std::endl;
                return;
            }
        ...
        usb_setup_t endControl(uint8_t endpoint, uint64_t id) {
            if (!this->m_inflight[endpoint].contains(id)) {
                std::cerr << "Control completion on endpoint " << std::dec << (uint32_t)endpoint << " could not find submit for " << std::hex << id << std::endl;
                return usb_setup_t{};
            }

            if (!this->m_controlSetup[endpoint].contains(id)) {
                std::cerr << "Control completion on endpoint " << std::dec <<  (uint32_t)endpoint << " with id " << id << " missing setup info" << std::endl;
                return usb_setup_t{};
            }
        ...
        void beginBulk(uint8_t endpoint, uint64_t id) {
            ...
            if (this->m_inflight[endpoint].contains(id)) {
                std::cerr << "Bulk already had inflight operation on " << std::dec << (uint32_t)endpoint << " with id " << std::hex << id << std::endl;
                return;
            }
            this->m_inflight[endpoint].insert(id);
        }
        ...
        void endBulk(uint8_t endpoint, uint64_t id) {
            if (!this->m_inflight[endpoint].contains(id)) {
                std::cerr << "Bulk completion on endpoint " << std::dec << (uint32_t)endpoint << " with id " << std::hex << id << " does not have matching submit." << std::endl;
                return;
            }
            this->m_inflight[endpoint].erase(id);
        }
